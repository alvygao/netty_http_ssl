...
public class ServletNettyHandler extends SimpleChannelInboundHandler<Object> {
	...
	public void messageReceived(ChannelHandlerContext ctx, Object msg)
			throws Exception {
		MockHttpServletRequest servletRequest = null;

		if (msg instanceof HttpRequest) {
			HttpRequest httpRequest = (HttpRequest) msg;
			servletRequest = createServletRequest(httpRequest);
		...
		if (msg instanceof HttpContent) {
			...
			servletRequest.setContent(content.toString(CharsetUtil.UTF_8)
					.getBytes());
		}

		handleRequest(servletRequest, ctx);
	}

	private void handleRequest(ServletRequest servletRequest,
		...
		FullHttpResponse response = new DefaultFullHttpResponse(HTTP_1_1,
				status, Unpooled.wrappedBuffer(servletResponse
